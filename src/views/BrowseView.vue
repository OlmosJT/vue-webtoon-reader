<template>
  <div class="browse-view">
    <MangaFilter @filter-change="updateFilters" />
...
const filteredManga = computed(() => {
  ...
  if (filters.value.searchQuery) {
    manga = manga.filter(m => m.title.toLowerCase().includes(filters.value.searchQuery.toLowerCase()));
  }

  if (filters.value.genres.length) {
    manga = manga.filter(m => filters.value.genres.every(g => m.genres.includes(g)));
  }

  if (filters.value.status) {
    manga = manga.filter(m => m.status === filters.value.status);
  }
