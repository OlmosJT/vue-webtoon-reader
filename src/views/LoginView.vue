<template>
  <div class="auth-container">
    <div class="auth-card">
      ...
      <form v-if="activeMethod === 'phone'" @submit.prevent="handleLogin" class="auth-form">
        <div class="form-group">
          <label for="phone">Phone Number</label>
          <input type="tel" id="phone" placeholder="+1 (555) 555-5555" required />
        </div>
        <button type="button" class="btn-submit-outline">Send OTP</button>
      </form>
...
const { login } = useMangaStore();
const router = useRouter();

const handleLogin = () => {
  login({ name: 'olmosjt', email: 'olmosjt@example.com' });
  router.push('/');
};
</script>
